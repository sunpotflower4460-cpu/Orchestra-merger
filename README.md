...
> 注意: Copilot への割り当ては PWA の「次の Issue を開始」または `orchestrate` workflow 経由に統一してください。GitHub UI から直接「Assign to Copilot」すると、`queued → in-progress` のラベル遷移が行われずキュー状態がズレます。万一直接割り当ててしまった場合でも、`orchestrate` 実行時に「`queued` のまま Copilot に割り当て済みの Issue」を検知して `in-progress` に補正し、重複割り当てを防ぎます。
